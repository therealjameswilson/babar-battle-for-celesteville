...
document.querySelector('iframe').onload=async()=>{
 const renderEnabled=new URLSearchParams(location.search).get('render')!=='off';
 ...
 g.eval(`
  this.benchRender=${renderEnabled};nextWave=enemySpawn=99999;revealUntil=99999;cam={x:950,y:650,zoom:.7};
  ...
  draw=function(){const start=performance.now();if(benchRender)originalDraw();if(bench.count>30)bench.draw.push(performance.now()-start);};
 ...
 const stats=g.eval(`(()=>{
   ...
   return {renderEnabled:benchRender,visibility:document.visibilityState,viewport:[canvas.clientWidth,canvas.clientHeight],survivingUnits:units.filter(u=>u.hp>0).length,elapsedMs:Math.round(performance.now()-bench.started),frameMs:summary(bench.frames),loopMs:summary(bench.loop),drawMs:summary(bench.draw),simulationMs:summary(bench.update)};
 })()`);
