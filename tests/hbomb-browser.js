(async()=>{let count=0;const lines=[];const check=(v,s)=>{if(!v)throw Error(s);count++;};try{
 ...
 button('Assemble H-bomb').click();check(g.eval('!!fab.atomicJob'),'assembly button');g.eval("updateAtomic(110);selected=[alive(0).find(u=>u.type==='hero')];updateUI(true)");check(!!button('Launch H-bomb'),'launch button');button('Launch H-bomb').click();check(g.eval("mode==='atomic'"),'aim mode');
