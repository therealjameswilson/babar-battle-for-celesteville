...
function defenseStep(excludedIds = [], allowAdvance = true, reserveSchool = false, reserveSupplies = 0) {
  ...
  function construct(type,p) {if(spendable()>= buildingCost(type)&&materials>=materialCost(type)&&validBuild(p,type)){selected=[base];build(type);command(p);}}
  ...
  if(workers.length+base.queue.length<11 && spendable()>= 50){selected=[base];train('worker');}
  ...
  if(!reserveSchool&&school&&!school.construction&&!own.some(u=>u.type==='scout')&&!school.queue.includes('scout')&&spendable()>= 55){selected=[school];train('scout');}
  const guns=own.filter(u=>u.type==='walker');
  const infantry=own.filter(u=>u.type==='trooper');
  if(factory&&!factory.construction&&factory.queue.length<2&&guns.length<10&&spendable()>=defs.walker.cost){selected=[factory];train('walker');}
  if(!reserveSchool&&school&&!school.construction&&school.queue.length<2&&infantry.length<24&&spendable()> (!factory&&materials>=50?300:factory&&!factory.construction&&guns.length<3?220:60)){selected=[school];train('trooper');}
  if(!benefits.has('celeste')&&spendable()> 200)usePower('celeste');
  if(!benefits.has('pompadour')&&spendable()> 250)usePower('pompadour');
