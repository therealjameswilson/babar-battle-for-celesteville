...
const OPENING_SCENES = [
  ...
  {tag:'III · THE RECKONING',title:'Two kings. One road home.',speaker:'CELESTEVILLE · BEFORE FIRST LIGHT',line:'One fights to claim a kingdom. One fights to keep his people together.',reply:'Their rivalry ends where your command begins.',side:'both'}
];
(() => {
  const dialog=document.createElement('dialog');dialog.id='opening';dialog.setAttribute('aria-labelledby','opening-title');
  dialog.innerHTML='<div class="opening-sky" aria-hidden="true"></div><div class="opening-rivals" aria-hidden="true"><div class="opening-babar">'+characterArtMarkup('babar')+'</div><div class="opening-rataxes">'+characterArtMarkup('rataxes')+'</div></div><div class="opening-body"><p id="opening-tag">A KINGDOM AT THE BREAKING POINT</p><div id="opening-copy" aria-live="polite"><h2 id="opening-title">Two crowns.<br>One border.</h2><p id="opening-speaker">BABAR · THE SIEGE OF CELESTEVILLE</p><p id="opening-line">Beyond the forest, Rataxes is gathering his army. In Celesteville, the last lamps still burn.</p><p id="opening-reply">An original wartime prologue.</p></div><div class="opening-progress" aria-label="Opening progress"><i></i><i></i><i></i></div><div class="opening-controls"><button id="opening-play">Begin opening</button><button id="opening-next" hidden>Next scene</button><button id="opening-mute"></button><button id="opening-skip">Skip to briefing</button></div><small id="opening-hint">Sound begins only after you choose Begin opening. About 24 seconds.</small></div>';
  ...
  function show(){
    const s=OPENING_SCENES[scene];dialog.dataset.side=s.side;dialog.dataset.motion=reducedMotion?'reduced':'full';
    ...
    $('opening-next').hidden=false;$('opening-next').textContent=scene===2?'Mission briefing':'Next scene';
  ...
  function next(){if(scene>=2){finish();return;}scene++;show();}
